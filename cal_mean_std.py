...
if __name__ == "__main__":
    ...
    # style_1 = np.dot((Ls-mean_s), np.transpose(std_s/std_c)) + mean_c
    # style_1 = np.reshape(np.array(style_1).astype(dtype=np.uint8), (h, w, 3))
    # style_1 = cv2.cvtColor(style_1, cv2.COLOR_YUV2RGB)
    # plt.imshow(style_1)
    # plt.show()

    # print(mean_s)
############################### Tính result
# Zc = covariances2(path_content)
# Lc = np.linalg.cholesky(Zc)
#
# Zs = covariances2(path_style)
# Ls = np.linalg.cholesky(Zs)
# Ls_1 = np.linalg.matrix_power(Ls, -1)
#
# A = np.dot(Lc, Ls_1)
# b = mean(path_content) - numpy.dot(A, mean(path_style))
#
# image = Image.open(path_style).convert('RGB')
# image = np.uint8(image)
# print(image)
# h, w, _ = image.shape
# style_copy = image.copy()
# image = np.concatenate(image, axis=0)
# # print(image.shape)
# # print(image)
# # tách kênh màu
...
# # print(result)
#
# img2 = np.reshape(np.array(result), (h, w, 3))
# print(img2)
# plt.imshow(img2[0])
# plt.show()
...
# Zc = covariances(path_content)
# Lc = np.linalg.cholesky(Zc)
# Zs = covariances(path_style)
# Ls = np.linalg.cholesky(Zs)
# Ls_1 = np.linalg.matrix_power(Ls, -1)
# A = np.dot(Lc, Ls_1)
# A = [[[206.11453037, 196.05055893, 189.34124463, 187.66391606],
#  [103.04513719,  95.06622779,  88.80588825,  85.38005804],
#  [ 69.84403991,  61.78418319,  56.76419476,  54.74616753]],
# [[206.11453037, 196.05055893, 189.34124463, 187.66391606],
# [103.04513719, 95.06622779, 88.80588825, 85.38005804],
# [69.84403991, 61.78418319, 56.76419476, 54.74616753]]
#      ]
# A = np.array(A)
